...
class FinancialApiTest extends TestCase
{
    ...
    public function test_authenticated_user_can_create_project_and_budget_line(): void
    {
        ...
        $project = $this->actingAs($user, 'sanctum')->withHeaders($headers)->postJson('/api/projects', [
            'code' => 'PROJ-TEST-001', 'name' => 'Projet de test', 'donor' => 'Bailleur', 'budget_amount' => 1000000, 'status' => 'active',
        ])->assertCreated()->json();

        $this->actingAs($user, 'sanctum')->withHeaders($headers)->postJson('/api/budgets', [
            'project_id' => $project['id'], 'code' => 'ACT-001', 'category' => 'Activités', 'allocated_amount' => 500000,
        ])->assertCreated()->assertJsonPath('project.id', $project['id']);
    }
    ...
    public function test_authenticated_user_can_submit_and_approve_an_expense(): void
    {
        [$user, $organization] = $this->context();
        $headers = ['X-Organization-Id' => (string) $organization->id];
        $transaction = $this->actingAs($user, 'sanctum')->withHeaders($headers)->postJson('/api/transactions', [
            'type' => 'expense', 'label' => 'Fournitures', 'amount' => 25000, 'currency' => 'XOF', 'document_url' => 'https://example.com/facture.pdf',
        ])->assertCreated()->json();

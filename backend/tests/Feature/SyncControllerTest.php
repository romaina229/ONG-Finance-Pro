...
class SyncControllerTest extends TestCase
{
    ...
    public function test_push_creates_transaction_and_is_idempotent(): void
    {
        ...
        $this->actingAs($user, 'sanctum')->withHeader('X-Organization-Id', (string) $organization->id)
            ->postJson('/api/sync/push', ['operations' => [$payload]])->assertOk()->assertJsonPath('data.0.status', 'accepted');
        $this->actingAs($user, 'sanctum')->withHeader('X-Organization-Id', (string) $organization->id)
            ->postJson('/api/sync/push', ['operations' => [$payload]])->assertOk()->assertJsonPath('data.0.status', 'accepted');
    ...
    public function test_update_with_stale_version_is_marked_as_conflict(): void
    {
        ...
        $response = $this->actingAs($user, 'sanctum')->withHeader('X-Organization-Id', (string) $organization->id)->postJson('/api/sync/push', ['operations' => [[
            'operation_id' => (string) Str::uuid(), 'organization_id' => $organization->id, 'entity_type' => 'financial_transaction', 'local_id' => $transaction->local_id, 'server_id' => $transaction->id, 'action' => 'update', 'base_version' => 1, 'payload' => ['label' => 'Modification offline'],
        ]]]);
    ...
    public function test_delete_increments_version_and_soft_deletes_transaction(): void
    {
        ...
        $this->actingAs($user, 'sanctum')->withHeader('X-Organization-Id', (string) $organization->id)->postJson('/api/sync/push', ['operations' => [[
            'operation_id' => (string) Str::uuid(), 'organization_id' => $organization->id, 'entity_type' => 'financial_transaction', 'local_id' => $transaction->local_id, 'server_id' => $transaction->id, 'action' => 'delete', 'base_version' => 1, 'payload' => [],
